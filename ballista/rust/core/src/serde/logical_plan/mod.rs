        ...
        match plan {
            ...
            LogicalPlanType::Projection(projection) => {
                let input: LogicalPlan =
                    into_logical_plan!(projection.input, ctx.clone(), extension_codec)?;
            ...
            LogicalPlanType::Selection(selection) => {
                let input: LogicalPlan =
                    into_logical_plan!(selection.input, ctx.clone(), extension_codec)?;
            ...
            LogicalPlanType::Window(window) => {
                let input: LogicalPlan =
                    into_logical_plan!(window.input, ctx.clone(), extension_codec)?;
            ...
            LogicalPlanType::Aggregate(aggregate) => {
                let input: LogicalPlan =
                    into_logical_plan!(aggregate.input, ctx.clone(), extension_codec)?;
            ...
            LogicalPlanType::ListingScan(scan) => {
                ...
                let object_store = RuntimeEnv::global()
            ...
            LogicalPlanType::Sort(sort) => {
                let input: LogicalPlan =
                    into_logical_plan!(sort.input, ctx.clone(), extension_codec)?;
            ...
            LogicalPlanType::Repartition(repartition) => {
                ...
                let input: LogicalPlan =
                    into_logical_plan!(repartition.input, ctx.clone(), extension_codec)?;
            ...
            LogicalPlanType::Analyze(analyze) => {
                let input: LogicalPlan =
                    into_logical_plan!(analyze.input, ctx.clone(), extension_codec)?;
                LogicalPlanBuilder::from(input)
            ...
            LogicalPlanType::Explain(explain) => {
                let input: LogicalPlan =
                    into_logical_plan!(explain.input, ctx.clone(), extension_codec)?;
                LogicalPlanBuilder::from(input)
            ...
            LogicalPlanType::Limit(limit) => {
                let input: LogicalPlan =
                    into_logical_plan!(limit.input, ctx.clone(), extension_codec)?;
                LogicalPlanBuilder::from(input)
            ...
            LogicalPlanType::Join(join) => {
                ...
                let builder = match join_constraint.into() {
                    JoinConstraint::On => builder.join(
                        &into_logical_plan!(join.right, ctx.clone(), extension_codec)?,
                        join_type.into(),
            ...
            LogicalPlanType::CrossJoin(crossjoin) => {
                let left = into_logical_plan!(crossjoin.left, ctx.clone(), extension_codec)?;
                let right = into_logical_plan!(crossjoin.right, ctx.clone(), extension_codec)?;
...
mod roundtrip_tests {
    ...
    async fn roundtrip_logical_plan_custom_ctx() -> Result<()> {
        ...
        RuntimeEnv::global().register_object_store("test", custom_object_store.clone());

        let (os, _) = RuntimeEnv::global().object_store("test://foo.csv")?;
