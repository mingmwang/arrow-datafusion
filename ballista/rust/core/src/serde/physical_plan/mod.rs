        ...
        match plan {
            PhysicalPlanType::Projection(projection) => {
                let input: Arc<dyn ExecutionPlan> =
                    into_physical_plan!(projection.input, session_id, extension_codec)?;
            ...
            PhysicalPlanType::Filter(filter) => {
                let input: Arc<dyn ExecutionPlan> =
                    into_physical_plan!(filter.input, session_id, extension_codec)?;
            ...
            PhysicalPlanType::Merge(merge) => {
                let input: Arc<dyn ExecutionPlan> =
                    into_physical_plan!(merge.input, session_id, extension_codec)?;
                Ok(Arc::new(CoalescePartitionsExec::new(input)))
            }
            PhysicalPlanType::Repartition(repart) => {
                let input: Arc<dyn ExecutionPlan> =
                    into_physical_plan!(repart.input, session_id, extension_codec)?;
            ...
            PhysicalPlanType::GlobalLimit(limit) => {
                let input: Arc<dyn ExecutionPlan> =
                    into_physical_plan!(limit.input, session_id, extension_codec)?;
            ...
            PhysicalPlanType::Window(window_agg) => {
                let input: Arc<dyn ExecutionPlan> =
                    into_physical_plan!(window_agg.input, session_id, extension_codec)?;
            ...
            PhysicalPlanType::HashAggregate(hash_agg) => {
                let input: Arc<dyn ExecutionPlan> =
                    into_physical_plan!(hash_agg.input, session_id, extension_codec)?;
            ...
            PhysicalPlanType::HashJoin(hashjoin) => {
                ...
                let right: Arc<dyn ExecutionPlan> =
                    into_physical_plan!(hashjoin.right, session_id, extension_codec)?;
            ...
            PhysicalPlanType::CrossJoin(crossjoin) => {
                ...
                let right: Arc<dyn ExecutionPlan> =
                    into_physical_plan!(crossjoin.right, session_id, extension_codec)?;
            ...
            PhysicalPlanType::ShuffleReader(shuffle_reader) => {
                ...
                let shuffle_reader =
                    ShuffleReaderExec::try_new(partition_location, schema, session_id)?;
            ...
            PhysicalPlanType::Sort(sort) => {
                let input: Arc<dyn ExecutionPlan> =
                    into_physical_plan!(sort.input, session_id, extension_codec)?;
                let exprs = sort
                    .expr
                    .iter()
                    .map(|expr| {
            ...
            PhysicalPlanType::Extension(extension) => {
                let inputs: Vec<Arc<dyn ExecutionPlan>> = extension
                    ...
                    .iter()
                    .map(|i| {
                        i.try_into_physical_plan(session_id.clone(), extension_codec)
                    })
                    .collect::<Result<_, BallistaError>>()?;
...
mod roundtrip_tests {
    ...
    fn roundtrip_empty() -> Result<()> {
        roundtrip_test(Arc::new(EmptyExec::new(
            false,
