...
mod tests {
    ...
    async fn test_extension_plan() -> crate::error::Result<()> {
        ...
        let ctx = Arc::new(SessionContext::with_state(state, RuntimeEnv::global()));
        ...
        let logical_round_trip = proto.try_into_logical_plan(ctx.clone(), &extension_codec)?;
        ...
        let physical_round_trip = proto.try_into_physical_plan(ctx.session_id.clone(), &extension_codec)?;
