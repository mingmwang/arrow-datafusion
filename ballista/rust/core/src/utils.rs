...
pub fn create_df_ctx_with_ballista_query_planner<T: 'static + AsLogicalPlan>(
    ...
    let mut session_state = ExecutionState::with_config(session_config).with_query_planner(planner);
    session_state.session_id = session_id;
    // the ExecutionContext created here is the client side context, but the session_id is from server side.
    let df_ctx = ExecutionContext::with_state(session_state, RuntimeEnv::global());
    df_ctx
}
    ...
    async fn create_physical_plan(
        ...
        match logical_plan {
            LogicalPlan::CreateExternalTable(_) => {
                ...
                Ok(Arc::new(EmptyExec::new(false, Arc::new(Schema::empty()), session_state.session_id.clone())))
            }
