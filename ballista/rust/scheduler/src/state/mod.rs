    ...
    StableSchedulerState<T, U>
{
    ...
    async fn init_stages_from_storage(&self) -> Result<()> {
        ...
        for (key, entry) in entries {
            let (session_id, job_id, stage_id) = extract_stage_id_from_stage_key(&key).unwrap();
            let value = U::try_decode(&entry)?;
            let plan = value
                .try_into_physical_plan(session_id.clone(), self.codec.physical_extension_codec())?;
    ...
    async fn save_stage_plan(
    ...
    ) -> Result<()> {
        {
            ...
            // Save in db
            let key = get_stage_plan_key(&self.namespace, session_id.as_str(), job_id, stage_id as u32);
            let value = {
...
fn get_stage_plan_key(namespace: &str, session_id: &str, job_id: &str, stage_id: u32) -> String {
    format!("{}/{}/{}/{}", get_stage_prefix(namespace), session_id, job_id, stage_id)
}
