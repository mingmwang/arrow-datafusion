...
mod tests {
    ...
    fn build_table(
        ...
        Arc::new(MemoryExec::try_new(&[vec![batch]], schema, None, session_id.to_owned()).unwrap())
    }
    ...
    async fn join_inner_one_two_parts_left() -> Result<()> {
        ...
        let left = Arc::new(
            MemoryExec::try_new(&[vec![batch1], vec![batch2]], schema, None, "sess_123".to_owned()).unwrap(),
        );
    ...
    async fn join_inner_one_two_parts_right() -> Result<()> {
        ...
        let schema = batch1.schema();
        let right = Arc::new(
            MemoryExec::try_new(&[vec![batch1], vec![batch2]], schema, None, session_id.to_owned()).unwrap(),
        );
    ...
    fn build_table_two_batches(
        ...
        let schema = batch.schema();
        Arc::new(
            MemoryExec::try_new(&[vec![batch.clone(), batch]], schema, None, "sess_123".to_owned()).unwrap(),
        )
    }
    ...
    async fn join_left_empty_right() {
        ...
        let right = Arc::new(MemoryExec::try_new(&[vec![right]], schema, None, "sess_123".to_owned()).unwrap());
        let join = join(left, right, on, &JoinType::Left, false).unwrap();
