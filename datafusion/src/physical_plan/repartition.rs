...
mod tests {
    ...
    async fn repartition(
        ...
        let exec = MemoryExec::try_new(&input_partitions, schema.clone(), None, session_id.to_owned())?;
    ...
    async fn error_for_input_exec() {
        ...
        let input = ErrorExec::new("error_session_id".to_owned());
    ...
    fn make_barrier_exec() -> BarrierExec {
        ...
        let schema = batch1.schema();
        let session_id= "sess_123";
        BarrierExec::new(vec![vec![batch1, batch2], vec![batch3, batch4]], schema, session_id.to_owned())
    }
    ...
    async fn test_drop_cancel() -> Result<()> {
        ...
        let blocking_exec = Arc::new(BlockingExec::new(Arc::clone(&schema), 2, session_id.to_owned()));
